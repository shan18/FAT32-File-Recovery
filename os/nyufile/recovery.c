...
void recover_contiguous_file(unsigned char *disk, BootEntry *disk_info, char *disk_name, char *file_name, char *sha1) {
    ...
    for (unsigned int next_cluster = disk_info->BPB_RootClus; next_cluster < 0x0ffffff8 && !file_found; next_cluster = *read_fat1(disk, disk_info, next_cluster)) {
        for (
            ...
            if (entry->DIR_Name[0] == 0xe5 && entry->DIR_Attr == 0x20) {
                if (compare_entries(entry->DIR_Name, file_name, 1)) {
                    if (write_disk(disk_name, next_cluster, (unsigned char) file_name[0]) == 0) {
